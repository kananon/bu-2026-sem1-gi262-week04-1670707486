...
namespace Assignment
{
    public class Assignment : MonoBehaviour
    {
        ...
        public void AS07_RemoveDuplicatesFromLinkedList()
        {
            ...
            if (list.Count <= 1)
            {
                LinkedListNode<int> printNode = list.First;
            ...
            LinkedListNode<int> finalNode = list.First;
            while (finalNode != null)
            {
                Debug.Log(finalNode.Value);
        ...
        public void AS10_GameEventQueue()
        {
            ...
            while (eventQueue.Count > 0)
            {
                ...
                string eventTypeLower = currentEvent.EventType?.ToLower();
                if (eventTypeLower == "enemy")
                {
                    Debug.Log($"Enemy event processed - {currentEvent.Name}");
                }
                else if (eventTypeLower == "powerup")
                {
                    Debug.Log($"Power-up event processed - {currentEvent.Name}");
                }
                else if (eventTypeLower == "level")
                {
